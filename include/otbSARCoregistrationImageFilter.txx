...
::SARCoregistrationImageFilter()
 {
  ...
  m_Interpolator = otb::ComplexInterpolateImageFunction<
		TInputImage, 
		otb::Function::BlackmanWindowFunction<typename PixelType::value_type >, 
...
::GenerateData()
 {
  ...
  if( m_UseDEM == true)
  {
      ...
      itkWarningMacro(<<"No member named SetDEMDirectory in otb::GenericRSTransform.");
  }
  ...
  while (gridIt != points->End() )
    {
		...
		typename TInputImage::PointType masterOrigin = masterPtr->GetOrigin();

		IndexType slaveIndex;
		slavePtr->TransformPhysicalPointToIndex(slavePoint, slaveIndex);

		typename TInputImage::PointType slaveOrigin = slavePtr->GetOrigin();
		...
		typename ExtractFilterType::Pointer masterExtract = ExtractFilterType::New();
		masterExtract->SetInput(masterPtr);
		masterExtract->SetExtractionRegion(masterCurrentRegion);

		typename ExtractFilterType::Pointer slaveExtract = ExtractFilterType::New();
		...
		typename PadFilterType::Pointer masterPad = PadFilterType::New();
		masterPad->SetInput(masterExtract->GetOutput());
		masterPad->SetPadBound(padSize);

		typename PadFilterType::Pointer slavePad = PadFilterType::New();
		...
		typename FFTOutputImageType::Pointer masterFFTImage = fft->GetOutput();

		fft = FFTType::New();
		fft->SetInput(slavePad->GetOutput());
		fft->Update();

		typename FFTOutputImageType::Pointer slaveFFTImage = fft->GetOutput();

		// Calculate complex conjugate product
		typename ConjugateProductFilterType::Pointer conjProduct = ConjugateProductFilterType::New();
		conjProduct->SetInput1(masterFFTImage);
		conjProduct->SetInput2(slaveFFTImage);

		// Pseudo normalize
		typename ModulusFilterType::Pointer conjProdModulus = ModulusFilterType::New();
		conjProdModulus->SetInput(conjProduct->GetOutput());

		typename DivideFilterType::Pointer conjProdNormalize = DivideFilterType::New();
		...
		typename FFTOutputImageType::Pointer invFFT = fft->GetOutput();
	...
	if( estimate->GetTiePointsContainer().size() != 0 )
	{
		...
		typename NormalizeZeroFrequencyType::Pointer normalizeZeroFrequency = NormalizeZeroFrequencyType::New();
