...
typename PlatformPositionToBaselineCalculator<TFunctor>::BaselineFunctorOutputType
PlatformPositionToBaselineCalculator<TFunctor>
::Evaluate(double line, double lineOffsetWithMaster, int map) 
{
	std::vector<double> masterPosition(3);
	std::vector<double> slavePosition(3);
	std::vector<double> masterSpeed(3);
...
{	
	...
	double minTangentialBaseline = std::abs(this->Evaluate(0.0, 0.0, TFunctor::Tangential));
	...
	while((minimumFound ==false) && (offsetValue < maxNumberOfRow))
	{
		...
		double tempTangentialBaseline = std::abs(this->Evaluate(0.0, offsetValue, TFunctor::Tangential));
